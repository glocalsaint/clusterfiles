...
public:
    ...
    void create_graphwithedgelists(vector<std::pair<string,int>> &flwords, map<int, std::vector<int>> &edgelists, unordered_map<string, int> &fl_frequencymap, int &myrank)
    {
        ...
        for(auto &entry : edgelists)
        {
            ...
            for(int i =0 ; i < edgelist_of_fromnode.size() ; i=i+2)
            {
                ...
                if(from_node->frequency == 0 || to_node->frequency == 0 ) continue;
                double weight= 1-std::max((double)cooccurance_count/from_node->frequency , (double)cooccurance_count/to_node->frequency);
                //cout <<"Weight:"<<weight<<" c: " << cooccurance_count << " f: " << from_node->frequency <<" t: " << to_node->frequency<<"\n";
        ...
        if (WRITE_OUTPUT_TO_FILE == 1)
                outputstream << "P:" << myrank << " ; EV: TWord: " << target_word << " ; " << " V: " << stringtonode_map.size() << " ; " << " E: " << no_of_edges ;
        else   
            cout << "P:" << myrank << " ; EV: TWord: " << target_word << " ; " << " V: " << stringtonode_map.size() << " ; " << " E: " << no_of_edges;
    ...
    void performMST()
    {
        ...
        while(treeset.size()!=stringtonode_map.size() && treeset.size()!=allzeroedges)
        {
            ...
            for(auto &entry: treeset)
            {
                ...
                if( min_weight_edge.second > firstedge->second){
                    ...
                    min_weight_edge.first = firstedge->first;
                    min_weight_edge.second = firstedge->second;
                    //cout <<min_weight_edge.second<<"|"<<endl;
                    from_node = nodeptr;
                    //nodeptr->edges.erase(firstedge);
            ...
            if(!(nodeptr->isroothub))
            {
                ...
                while(1)
                {
                    ...
                    {                        
                        ...
                        //cout << "Came score: "<<nodeptr->score<< " totalweight:" << totalweight << "\n";
        ...
        for(auto &entry: stringtonode_map)
        {
            ...
            int roothubnum = nodeptr->roothubnum;
        	//stringtoscore_map[entry.first]= std::make_pair(nodeptr->score, nodeptr->roothubnum);
            //delete nodeptr;
            string sqlstatement = "INSERT INTO scoreroothub values ('"+target_word+"','" + entry.first + "'," + to_string(score) + "," + to_string(roothubnum) + ");";
             /* Execute SQL statement */
           rc = sqlite3_exec(db, sqlstatement.c_str(), 0, 0, &zErrMsg);
           if( rc != SQLITE_OK ){
           fprintf(stderr, "SQL Insert error: %s\n", zErrMsg);
              sqlite3_free(zErrMsg);
              exit;
           }else{
              //fprintf(stdout, "Row Insert successfully\n");
           }
        }
